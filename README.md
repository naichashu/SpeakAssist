...
也可以换成任何 OpenAI 兼容的视觉多模态接口，但**通用模型（GPT-4o / Claude / Gemini 一类）实测效果远不如 AutoGLM-Phone**——后者是针对手机自动化场景专门训练的，坐标、控件识别、任务终止判断都更准。详见下方 FAQ「能换别的模型吗」。
...
**架构上支持，实际上不建议**。客户端可以接任何 OpenAI 兼容的视觉多模态 API（在"API 配置"里改 Base URL 和 Model Name 即可），但**真正能把这套任务循环跑稳的，目前只有智谱 AutoGLM-Phone**。

原因是 AutoGLM-Phone 是**针对手机 UI 自动化专门训练**的模型——它的训练分布里就有"截屏 + 屏幕信息 → 输出单个动作"这种格式，对手机屏幕坐标、控件语义、任务终止判断都熟。

通用模型（GPT-4o、Claude、Gemini 一类）实测表现：能看懂截图、能写出符合格式的动作，但坐标经常偏、`finish` 时机也不对、对中文 App 的控件认知有限，跑下来成功率远低于 AutoGLM。

如果想试别的模型，建议挑专门为 GUI / 手机自动化做过 SFT 的模型，普通通用多模态意义不大。

#### Q: 想自己研究代码

直接读 `app/src/main/java/com/example/` 各包下的源文件。每个文件顶部都有功能注释，关键流程见：

- 任务循环主线：`ui/viewmodel/ChatViewModel.kt`
- 动作解析与执行：`register/ActionExecutor.kt`
- 模型客户端：`network/ModelClient.kt`
- 手势注入与机型适配：`service/MyAccessibilityService.kt`、`service/GestureTimingProfile.kt`
- 语音子系统：`speech/` 包下 `BaiduSpeechManager`、`WakeWordListeningManager`、`AdaptiveVad` 等
...
- **大模型**：智谱 AutoGLM-Phone（推荐，专为手机自动化训练）；客户端协议层兼容任何 OpenAI 多模态接口
