...
- `app/src/main/java/com/example/speakassist/MainActivity.kt`: main chat-style UI, permission banner, drawer navigation, voice/text input, and display of final task results. Input bar has two modes toggled by `btnInputMode`: keyboard (default, `tilInput` + `btnSend`) and voice (`btnPressToTalk` "按住 说话"). Press-to-talk is implemented by an inner-class `PressToTalkController` that drives an `OnTouchListener` state machine (DOWN → start, MOVE > 50dp up → "will cancel", UP → stop/cancel based on state, CANCEL → cancel) and shows a centered `voiceBubble` overlay with a `WaveformView` audio bar animation. After successful recognition the activity auto-switches back to keyboard mode and fills `etInput` with the recognized text.
...
- `app/src/main/java/com/example/speech/`: speech I/O. Two complementary engines, each with a single responsibility:
  - `BaiduSpeechManager` — online REST recognizer (百度语音). Handles all general-purpose command recognition: button-initiated capture from `MainActivity`, floating-window button capture, and the post-wake-word command capture. Returns null for empty/blank `result` so callers see `onError("未识别到语音")` instead of dispatching empty text. Exposes two `start()` overloads: `start()` (default `autoStopOnSilence=true`, used by floating-window/wake-word — VAD silence detection auto-stops recording) and `start(autoStopOnSilence: Boolean)` — when `false`, recording is **not** auto-stopped on silence and only ends via external `stop()/cancel()` or the 60s `HOLD_TO_TALK_MAX_DURATION_MS` fallback. The press-to-talk path in `MainActivity` uses `false`; everything else relies on the default.
