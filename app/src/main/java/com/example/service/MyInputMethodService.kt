...
class MyInputMethodService: InputMethodService() {
    ...
    companion object {
        ...
        fun isEnabled(context: Context): Boolean {
            ...
            val myImeName = "${context.packageName}/com.example.service.MyInputMethodService"
    ...
    override fun onDestroy() {
        super.onDestroy()
        instance = null
        Log.d(TAG, "MyInputMethodService 已销毁")
    }
    ...
    fun typeText(text: String): Boolean {
        ...
        if (inputConn == null) {
            Log.e(TAG, "输入失败：无可用的输入连接（未激活输入框）")
            return false
        }
